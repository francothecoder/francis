...
if ($reference) {
    ...
    if ($paymentId > 0) {
        ...
        $status = (string) ($data['data']['status'] ?? ($eventName === 'transaction.successful' ? 'successful' : ($eventName === 'transaction.failed' ? 'failed' : '')));
        if ($status === 'successful') {
            $pdo->prepare("UPDATE payment_transactions SET status='paid', paid_at = NOW() WHERE id = :id")->execute(['id'=>$paymentId]);
            $paymentRowStmt = $pdo->prepare('SELECT * FROM payment_transactions WHERE id = :id LIMIT 1'); $paymentRowStmt->execute(['id'=>$paymentId]); $payment = $paymentRowStmt->fetch();
            if ($payment) { if (!empty($payment['plan_id'])) activate_subscription_from_payment($paymentId); else activate_session_from_payment($paymentId); }
