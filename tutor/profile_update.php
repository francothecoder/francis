...
if ($headline === '' || $subjects === '' || $bio === '' || $startingPrice <= 0 || $minOfferPrice <= 0) {
    set_flash('error', 'Complete all tutor profile fields.');
    redirect_to('tutor/dashboard.php');
}

$stmt = $pdo->prepare('UPDATE tutor_profiles SET headline = :headline, subjects = :subjects, bio = :bio, starting_price = :starting_price, min_offer_price = :min_offer_price WHERE user_id = :user_id');
$stmt->execute([
    'headline' => $headline,
    'subjects' => $subjects,
    'bio' => $bio,
