...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name === '' || !validate_email($email) || !ensure_password_strength($password) || !in_array($role, ['student', 'tutor'], true)) {
        set_flash('error', 'Complete the form correctly. Password must be at least 8 characters.');
    ...
    $stmt = $pdo->prepare('INSERT INTO users (name, email, password, role, university, bio, avatar_path) VALUES (:name, :email, :password, :role, :university, :bio, :avatar_path)');
    $stmt->execute([
        'name' => $name,
        'email' => $email,
    ...
    if ($role === 'tutor') {
        $pdo->prepare('INSERT INTO tutor_profiles (user_id, headline, bio, subjects, starting_price, min_offer_price, is_verified) VALUES (:user_id, :headline, :bio, :subjects, 25.00, 15.00, 0)')
            ->execute([
                'user_id' => $userId,
